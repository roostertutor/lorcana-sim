...
- **cards** — **2325 named-ability cards wired + 539 vanillas = 2864/2865.** All of sets 1–11 + promos (P1, P2, P3, cp, DIS, D23) + set 12 (213 cards — Ravensburger has mirrored most of the set as of 2026-04-23). Promo sets auto-synced from main sets via `scripts/sync-promo-reprints.ts`.
- **gaps** — 1 stub remaining: Dale — Ready for His Shot (set 12 #22). "During challenges, your characters deal damage with their {W} instead of their {S}" — needs a CRD 4.6.6 challenge-damage stat-source override static, same family as Moana `ink_from_discard` / Donald `modify_win_threshold`. 0 partial, 0 invalid-fields, 0 known approximations. See the Audit section below for how the four audit scripts triangulate.
