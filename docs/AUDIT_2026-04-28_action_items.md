...
| **P1.13** | 🔄 | Add structured `cause` field to banish log so simulator/replay can disambiguate (challenge / damage / banish-effect / CRD 8.5.4 cleanup) without prose parsing. | ~1 hr engine + ~30 min UI | engine-expert + gameboard-specialist | engine `2026-04-28_engine_log_undo_rl.md` Topic 1 | Engine portion shipped 2026-04-28: new `BanishCause = "challenge" \| "damage" \| "banish_effect" \| "gsc_cleanup"` type union; optional `cause?: BanishCause` field on `GameLogEntry`; threaded through `banishCard` and explicit pass-through at GSC site for the challenge-vs-damage distinction. All 7 `banishCard` call sites classified cleanly (1 GSC explicit; 6 default to `"banish_effect"`). UI portion still open — gameboard-specialist needs to render the cause when present (e.g. "X was banished by challenge"). Suggested mapping in engine-expert handoff: challenge=sword/shield, damage=counter, banish_effect=current default, unknown=fallback. |
