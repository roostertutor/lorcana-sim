...
|---|---|---|---|---|---|---|
| **P0.1** | ☐ | **REAL SHIPPED UI HANG, not future-defense.** Audit 2026-04-28 found 7 cards live-surface `choose_player` (Lead the Way + 2 reprints, Mad Hatter "WE'LL HAVE TO LOOK INTO THIS", Copper Hound Pup "FOUND YA", "Change the Outcome", Sail to Neverland-style scry-4) and 6 cards live-surface `choose_card_name` in interactive mode (Merlin Clever Clairvoyant + variant, Bruno Madrigal + reprint, Blast from Your Past, Magic Brooms, D23 promo). Both auto-resolve in some narrow cases (single valid target / non-interactive sim), but surface a hung modal in 2P live play for the non-degenerate paths. Ship a 3-layer defense: **(1)** real UI input controls — player-picker (2 buttons labeled with player display names) + card-name input (text or autocomplete from `Object.values(definitions)`); **(2)** compile-time `never`-exhaustiveness in `PendingChoiceModal.renderContent()` so future variant additions to the union fail typecheck; **(3)** CLAUDE.md rule about engine→UI PendingChoice parity. Layer 2 (formerly proposed as "silent log + null") downgrades to a **last-resort runtime breadcrumb** behind layers 1 + the new UI branches; never user-visible. | ~2-3 hrs UI + ~30 min defense layers | gameboard-specialist (UI controls + exhaustiveness check) | gameboard `2026-04-28_modal_strings.md` + 2026-04-28 audit by engine-expert | Audit citations: `surfaceChoosePlayer` at `reducer.ts:8290-8321`; `name_a_card_then_reveal` constructor at `reducer.ts:3792-3818` (interactive-only) + consumer at `:2533`. Tests already exist at `set4.test.ts:194-236`. Simulator's `choiceResolver.ts` falls through generic handler for `choose_player` (works because targets are PlayerIDs); `choose_card_name` skipped by sim's non-interactive guard. |
