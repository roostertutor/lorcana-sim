...
| **P1.8** | ✅ | Replace `SAMPLE_DECKLIST` (15 hardcoded card-name lines in `DecksPage.tsx:24-39`) with engine helper `getSampleDeck(format)`. Eliminates the "Load sample" drift risk. | ~20 min UI + ~20 min engine helper | ui-specialist + engine-expert | ui non-gameboard `2026-04-28_modal_strings_non_gameboard.md` §10 | Shipped 2026-04-28. Resolution diverged from the audit's original "replace with engine helper" plan: removed the entire signed-out paste-and-analyze pane instead. Per-user call: non-authenticated visitors don't need a paste-analyze flow when the Sandbox tab is one click away in the nav. Eliminates `SAMPLE_DECKLIST` drift risk + drops the unused signed-out feature. Signed-out branch now just renders a sign-in prompt + sandbox CTA. Removed: `SAMPLE_DECKLIST` constant, paste state, `CompositionView` import, `useMemo` import. |
| **P1.9** | ✅ | Audit `extractOptionTexts` rulesText parsing. When parse fails, fall back to `"Option N"` labels rather than ship malformed text. | ~45 min | gameboard-specialist | gameboard `2026-04-28_modal_strings.md` | Shipped 2026-04-28 in `1171f77`. Both parser paths gated through a `looksSensible` predicate that rejects results unless every label is non-empty and ≤120 chars. Falls back to generic `"Option 1"`, `"Option 2"`, ... matching `pendingChoice.options.length` on any failure. The right long-term fix is engine-side typed labels — parked in BACKLOG. |
...
| **P1.11** | ✅ | Add log lines for effect-driven mutations (discard, look_at_hand, gain_lore, damage, move_damage, add_ink_from_hand) — currently silent. Players can't reconstruct why hand/lore/damage changed. | ~2-4 hrs (touches every effect handler) | engine-expert | engine `2026-04-28_engine_log_undo_rl.md` Topic 1 | Shipped 2026-04-28. New log lines for `discard_from_hand`, `look_at_hand` (privateTo looker; redacted server-side), `reveal_hand` (public), `gain_lore`/`lose_lore`, `deal_damage` non-challenge, `move_damage` all branches, `put_into_inkwell` from effect (privateTo inker; CRD 4.1.4 face-down). New `GameLogEntryType` variants: `lore_lost`, `card_discarded`, `damage_dealt`, `damage_moved`, `hand_revealed`. Server `stateFilter.ts` extended with redaction branches for `card_put_into_inkwell` (effect-driven) and `hand_revealed` (look_at_hand path). |
| **P1.12** | ✅ | Use `ability.storyName` in activated-ability log at `reducer.ts:1710` — currently `"X activated an ability on Y"` drops which ability. | ~20 min | engine-expert | engine `2026-04-28_engine_log_undo_rl.md` Topic 1 | Shipped 2026-04-28. Now reads `${playerId} activated ${ability.storyName ?? "an ability"} on ${def.fullName}` — surfaces e.g. "WAVE THE SCEPTER" / "INCREDIBLE ENERGY" / "YOU JUST HAVE TO SEE IT". Mirrors the trigger-log pattern already at `reducer.ts:6463`. Cards without `storyName` preserve the previous "an ability" wording (no regression). |
| **P1.13** | ✅ | Add structured `cause` field to banish log so simulator/replay can disambiguate (challenge / damage / banish-effect / CRD 8.5.4 cleanup) without prose parsing. | ~1 hr engine + ~30 min UI | engine-expert + gameboard-specialist | engine `2026-04-28_engine_log_undo_rl.md` Topic 1 | Engine `27689ae`: new `BanishCause = "challenge" \| "damage" \| "banish_effect" \| "gsc_cleanup"` + optional `cause?: BanishCause` on `GameLogEntry`; threaded through `banishCard`, explicit pass-through at GSC site for challenge-vs-damage distinction. UI `1171f77`: GameBoard's two log-render sites now show " — by challenge" / " — by damage" suffix in gray-600 metadata color; banish_effect/gsc_cleanup/undefined preserve current rendering. |
| **P1.14** | ✅ | **Migrate every targeting prompt to the `choose_may` gold-standard pattern** — engine passes `def.fullName + ability.storyName + ability.rulesText` to all prompt builders. Eliminates indistinguishable simultaneous prompts. | ~half-day engine | engine-expert | gameboard `2026-04-28_modal_strings.md` | Shipped 2026-04-28 in `cc61c3d`. New `AbilitySource` type + `buildPrompt(state, sourceInstanceId, definitions, abilitySource, verb)` helper. Format mirrors gold standard: `${def.fullName} — "${storyName}": ${rulesText}\n${verb}` with action-card / no-storyName / defensive fallbacks. **53 prompt sites migrated** (audit estimated ~50). Sites NOT migrated (mulligan, choose_play_order, bag-trigger ordering, free-play/alt-shift cost flows, floating triggers, mid-resolution recursive applyEffect) documented in `AbilitySource` JSDoc. Engine 720→723 tests. Mobile risk: longest prompt ~165 chars — flagged for gameboard follow-up if portrait overflow surfaces in playtests. |
...
| **P2.19** | ✅ | Consolidate hand-rolled modals through `ModalFrame` — DeckBuilderPage's discard-changes + box-art picker currently hand-roll the backdrop. | ~30 min | ui-specialist | ui non-gameboard §10 | Shipped 2026-04-28 in `69bde2b`. Both DeckBuilderPage modals (discard-changes + box-art picker) migrated. Adjacent modals (PendingChoiceModal, GameOver, BoardMenu, CardPicker, DeckBuilder Import/Export, FormatPicker, CardFilterBar) classified out-of-scope with reasoning in commit message. Behavior upgrade: dismiss semantics now go through ModalFrame's safer `e.target === e.currentTarget` check (iOS scroll-momentum fix). |
| **P2.20** | ✅ | Add regression test for undo-after-may-trigger invariant — assert that after `PLAY_CARD` of any card with optional triggers, an undo returns to the may prompt (not pre-play). Catches the "drop `isMay` and undo silently collapses" coupling. | ~30 min | engine-expert | engine `2026-04-28_engine_log_undo_rl.md` Topic 2 | Shipped 2026-04-28 in `e0c4acd`. Three tests in reducer.test.ts: (1) Diablo-Maleficent's-Spy isMay split + undo returns to may prompt; (2) The White Rose mandatory trigger does NOT split — undo collapses to pre-play (documents the asymmetric shape); (3) Diablo isMay decline-path also splits, pinning that boundary is at pendingChoice surface not "did effect fire". Engine 723→726. If `isMay` is dropped from Diablo, test 1 fails immediately on `pendingChoice?.type === "choose_may"` assertion. |
| **P2.21** | ✅ | Audit `useActiveEffects` label builder for paraphrase risk — flagged by gameboard-specialist as similar in shape to `formatDuration` / `filterLabel`. | ~30 min | gameboard-specialist | gameboard `2026-04-28_modal_strings.md` | Audit shipped 2026-04-28 (research only — no code changes). The hook is actually `getActiveEffects` (a function in `GameBoard.tsx:225-416`, not a hook). Verdict: mostly clean — 3 minor issues + 1 serious. Dominant pattern is verbatim ability-text passthrough (refutes the audit's "similar in shape to filterLabel" hypothesis). **Real bug found**: `formatEffectDuration` at `GameBoard.tsx:192-204` is a near-duplicate of `formatDuration` at `CardInspectModal.tsx:370-381` — same caster-perspective drift. Filed as **P2.21a** below. |
| **P2.21a** | ☐ | Consolidate `formatDuration` (`CardInspectModal.tsx:370-381`) + `formatEffectDuration` (`GameBoard.tsx:192-204`) into one shared helper that takes `(duration, casterPlayerId, viewerPlayerId)`. Both have the same caster-perspective drift bug — `until_caster_next_turn` renders as "Until your next turn" but is wrong when shown to the opponent of an opponent-targeted effect. While in there: (a) drop heuristic `+s` plural in play-restriction fallback at `getActiveEffects:319` (`Item/Locations` reads wrong); (b) replace raw discriminator leaks at `:299, :331, :400` (`"modify strength"` etc.) with cleaner fallbacks. | ~30-45 min | gameboard-specialist | P2.21 audit finding | New item filed 2026-04-28 from P2.21 audit; not in original audit scope. |
