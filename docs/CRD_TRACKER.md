...
| **1.9.5** | Damage reduced to 0 by modifiers = the character **was not dealt damage** | ✅ | Full audit of every deal-damage path — all gate on actual (post-modifier) damage > 0: (1) effect damage — `damage_dealt_to` trigger (8777) + `aCharacterWasDamagedThisTurn` flag (8763); (2) challenge damage — the whole block incl. `damage_dealt_to` AND the attacker-side `deals_damage_in_challenge` trigger sits inside `if (actual*Damage > 0)` (reducer.ts:1744-76). `move_damage`/`put_damage` are separate (not "dealt") and correctly don't fire deal-triggers. The raw `damage_dealt` game EVENT still emits at amount 0 in the effect path (8758) — cosmetic only (consumed by UI/log/tests, never by trigger logic). Tests: reducer.test.ts "CRD 1.9.5 … damage fully absorbed by Resist" (effect path) + "… does NOT fire the attacker's 'deals damage in a challenge' trigger" (Mulan - Elite Archer, the load-bearing case). |
