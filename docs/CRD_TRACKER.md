...
| Rule | Quote | Status |
|------|-------|--------|
| 6.5.1 | Replacement effects wait for a condition and partially/completely replace the event as it resolves | ✅ Multiple patterns implemented: `damage_redirect` (Beast), `damage_prevention_static` (Baloo, Hercules, Lilo — incl. `chargesPerTurn`), `challenge_damage_prevention` (Raya), `self_replacement` (48 cards), `self_banish_replacement` (Mickey & Minnie THINKING OF YOU — "if this would be banished, into your inkwell instead"; static on the card, intercepted in `banishCard` so it's source-agnostic across lethal-damage/challenge/effect banishes). No single unified "general replacement system" — each pattern is a distinct effect/modifier — but every card that needs one is wired. |
...
| **1.9.5** | 0-damage: **"deals"** still counts, **"takes"** does not | ✅ (fixed) | 1.9.5 has TWO halves that behave OPPOSITELY (my first pass had it backwards — one-sided "0 = not dealt"): (a) **"deals" (source side)** — if an effect/character WOULD deal damage (base > 0) but a modifier reduces it to 0, it is **still considered to deal damage**, so source-side triggers FIRE (CRD example = Merida - Formidable Archer STEADY AIM off a fully-Resisted action). (b) **"takes" (recipient side)** — a character whose incoming damage is reduced to 0 **takes no damage**, so recipient "is dealt / takes damage" triggers, counters, and the damaged-this-turn flag do NOT fire. Fix: `dealDamageToCard` fires `damage_dealt_to` on base `amount > 0` with a `dealtButNotTaken` flag (reducer.ts:8789); `queueTrigger` skips recipient self-triggers + cross-card no-`sourceFilter` watchers when `dealtButNotTaken`, but fires source-side `sourceFilter` watchers (STEADY AIM). Challenge: `deals_damage_in_challenge` fires on base Strength > 0 (reducer.ts:1777-91), recipient side stays on actual > 0. **Damage prevention** ("takes no damage") also fires the deals-side before the early-return (reducer.ts:8730/8745, both the charge-based static and timed paths). Tests: reducer.test.ts "CRD 1.9.5 … STILL 'deals' damage — STEADY AIM fires (user scenario)" (Resist), "… an action PREVENTED to 0 still 'deals' damage — STEADY AIM fires (Lilo - Bundled Up)" (prevention → lethal STEADY AIM banishes the shielded target), "… STILL fires the attacker's 'deals damage in a challenge' trigger" (Mulan), "… damage fully absorbed by Resist" (takes side / flag). |
...
| **4.5.1.2** | Quest now "**pays any costs** required to quest" + checks **limiters** (was "restrictions") | ✅ | `applyQuest` calls `payActionUnlockCost(…, "quest", …)` (reducer.ts:1396) — the "can't quest unless you pay N" family; limiters (Reckless, drying) enforced in `validateQuest` (validator.ts:599-600). No card yet costs ink to quest beyond the unlock family, but the step is wired. |
| **4.6.4.3** | Challenge now "check **requirements and limiters**, make choices, **pay any costs** to challenge" (was "restrictions") | ✅ | `applyChallenge` calls `payActionUnlockCost(…, "challenge", …)` (reducer.ts:1518, RC Remote-Controlled Car "can't challenge unless you pay 1"); Evasive/limiter checks in `validateChallenge`. |
| **4.6.4.4** | A **readied challenger stays the challenging character** and isn't removed from the challenge | ✅ | The Damage step only skips a combatant that **left play** (reducer.ts:1597-1601), never one that's readied — the challenge is keyed on `attackerInstanceId`, not exert state. Test: reducer.test.ts "CRD 4.6.4.4 … a challenger readied mid-challenge stays the challenging character" (Ming Lee - Giant Red Panda PATH OF DESTRUCTION readies her during Declaration; she still takes Hades's Strength in the Damage step). |
| **4.7.3.2–.4** | Move-a-character cost overhaul: announce move-cost vs **alternate cost**, determine total (cost + **payment modifiers**), then pay | ✅ | Move cost runs through `applyMoveCostReduction` (payment modifiers: `move_to_self_cost_reduction`, `global_move_cost_reduction`) + "for free" (reducer.ts:1817-43). No card has a non-free *alternate* move cost yet; the reduction/for-free framework is present. |
| **6.1.16.1–.3** | **Conditional dependent effects** — generalizes old 6.4.4.x (conditional *static* abilities) to **all** effects: "[Effect] unless/if [Condition]" | ✅ | The `condition` field is honored on static, **triggered**, and **activated** abilities alike (Set 13 alone has 59 conditioned abilities, incl. triggered `all_inkwell_exerted` on Randall DEVIOUS PLAN, activated on Absorbing Bloom METAMORPHOSIS). "unless" via `not(condition)` (Lilo - Bundled Up EXTRA LAYERS). |
...
| **6.1.3.1 / 6.7.3–.3.3** | Formal **requirements vs limiters** framework + resolution order (pay ink → make choices → check limiters legal) | ✅ | Matches: `validateAction` gates affordability up front; `choose`/`sequential` surface choices; `validateResolveChoice` enforces limiters (stat/cost caps) on the selection. Terminology reconciliation — behavior unchanged. |
...
| 5.6.6 | Characters **remain in play** when their location leaves play | ✅ | A location leaving play only **clears** hosted characters' `atLocationInstanceId` pointer — it does not banish them (reducer.ts:8285-90). |
