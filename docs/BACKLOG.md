...
- **B. Deckbuilder mobile pass** — **DONE 2026-05-25**, commit [pending]. Six scope items shipped: (1) card grid columns by viewport already used `auto-fill, minmax(100px, 1fr)` giving 3-col phone / 6-8 desktop — kept as-is, paired with 44×44px tap-target floor on `CardTile` stepper buttons (was 24px height). (2) `CardFilterBar` mobile filter sheet migrated from bespoke fixed-overlay to `ModalFrame` with `variant="auto"` + `MODAL_SIZE.lg`, picking up ESC-stack / scroll-lock / focus-restore / ARIA from Section C for free; trigger button bumped to 44×44px min. (3) `DeckBuilderPage` decklist sidebar collapses to a phone-only fixed bottom chip ("Decklist 42/60 · 1234 cost") that opens a bottom-sheet modal (`variant="auto"` + `MODAL_SIZE.lg`) containing the full `DeckBuilder` rows; `md:hidden` on the chip, `hidden md:block` on the inline rows. (4) `CardInspectModal` already conformed to Section C tokens (`variant="auto"` + `MODAL_SIZE.sm`) per the ModalFrame doc's single-card-detail mapping — verified no migration needed. (5) Tap-to-add already lived on `CardTile` (click art → add); tap-to-remove kept the explicit `[−][+]` stepper on `DeckBuilder.DeckRow` since the existing pattern works fine on touch — bumped to 44×44px on mobile via `min-w/h-[44px] md:min-w-0 md:min-h-0`, preserving desktop's compact 24px chips. (6) `DeckBuilder` deck-export PNG modal gains a phone-only Share button via `navigator.share({ files: [PNG] })` (`html-to-image` `toBlob`), capability-detected at module-eval with a stub `File` probe (mirrors `MultiplayerLobby` lobby-share pattern from `ca4e434`); falls through to download on share failure, silent on user cancel. No new modal infra introduced; reuses Section C tokens exclusively. No D/E/F creep.
...
- **D. Navigation chrome** — **DONE 2026-05-25**, commit [pending]. Bottom tab bar shipped 2026-05-03 (separate entry below). D1 (icons under tab labels) shipped: top-nav (md+) buttons now render Heroicons inline-left of labels at `App.tsx:534-547` (rectangle-stack / clock / play / wrench / user-circle); bottom nav already had icons stacked-above-label. D2 (back-button / swipe-back audit): walked through routes — `/replay/:gameId` player-only back path corrected from `/multiplayer` → `/replays` at `App.tsx:787-797`; other routes all behave correctly (`/game/:gameId` → `/multiplayer`, `/solo` → `/multiplayer`, `/sandbox/play` → `/sandbox`, `/lobby/:code` uses `replace: true` so back skips join page, `/play/:lobbyId` has inline Cancel/Leave, public `/replay/share/:replayId` stays `/` — right call given no caller context). DeckBuilderPage already has `← My Decks` link at `DeckBuilderPage.tsx:344-350`. No dead-end routes found.
- **E. Forms & input polish** — **DONE 2026-05-25** (remainder), commit [pending]. `inputMode` / `autoComplete` audit shipped 2026-05-03 (`66e84c7`). Remainder this session: E1 (hover-affordance audit) — grepped `group-hover:`, `hover:opacity-`, `hover:visible`, `peer-hover:`, `invisible` across `packages/ui/src/`; only one true hover-gated affordance found — box-art-picker "Change" caption at `DeckBuilderPage.tsx:459-465`, now `opacity-100 md:opacity-0 md:group-hover:opacity-100` (always visible on phone, hover-reveal on md+). All other `hover:` styles were color/bg shifts on already-visible elements (touch-safe). E2 (`DevAddCardPage` numeric inputs) skipped per dispatch — low ROI, internal tooling.
- **F. History / replays / decks** — **DONE 2026-05-25**, commit [pending]. F1 (DecksPage card density): migrated from inline `gridTemplateColumns` to `grid-cols-1 sm:grid-cols-[repeat(auto-fill,minmax(160px,1fr))]` at `DecksPage.tsx:96-104` — phone is now a stacked 1-col list (richer per-tile detail), tablet+ keeps the auto-fill grid with 160px min (2-col tablet / 3-4-col desktop). Untested visually in browser — user to sanity-check in `pnpm dev` (Tailwind 3.4 arbitrary-value syntax with commas should work). F2 (ReplaysPage row tap targets): `ReplayRow` button bumped from `py-2.5` (~32px) to `min-h-[56px] py-3` at `ReplaysPage.tsx:260-265`; "Load more" got `min-h-[44px]` at `:200-211`. Replay rows are static link cards, no per-row scrub-snap, so nothing else to do. F3 (ComparisonView swipe vs side-by-side): verified `ComparisonView.tsx:116` already uses `flex gap-4 flex-col sm:flex-row` (stacks on mobile, side-by-side on tablet+) and CompositionView uses `grid-cols-1 lg:grid-cols-2` — no code change needed.
...
**Trigger to reconsider**: **Mobile UX punch list COMPLETE as of 2026-05-25.** All six sections (A, B, C, D, E remainder, F) shipped across three pass cycles. Future mobile work would be new feature surfaces (new pages, new modals) — those should reuse Section C's tokens (`MODAL_SIZE` / `variant="auto"` / form-input pattern) and the 44×44px tap-target floor from Section A2. File new entries for specific surface gaps as they surface; this umbrella entry is done.

Suggested order (~~A~~ ~~B~~ ~~C~~ ~~D~~ ~~E remainder~~ ~~F~~ — all shipped 2026-05-03 / 2026-05-25): **complete**.

**Expected scope**: Each section ran roughly the size of Section C — 1–2 sessions, 2 commits (foundation + migrations or audit + fixes). Total cumulative: ~5 dispatches across the punch list (A1+A2 = 1, B = 1, C = 1, D+E remainder+F combined = 1, plus the bottom-nav standalone entry).
