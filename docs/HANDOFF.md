...
## UI agent + engine agent (atomic): Beyond the Horizon `choose_players_subset` UI branch + card-data swap

**Engine support landed in commit d16d379** (2026-04-30): added
`each_player.scope === "chosen_subset"` and the
`choose_players_subset` `PendingChoice` variant, with full reducer
+ validator + bot resolver + tests (5 new). Beyond the Horizon (set
8/202) is the only card in the corpus with caster-decides-which-
players wording: "Choose any number of players. They discard their
hands and draw 3 cards each."

**Why the wiring isn't swapped yet:** per CLAUDE.md "Engine→UI
PendingChoice parity," shipping a card that surfaces a new
PendingChoice variant without a UI branch causes the modal to fall
through to the generic targeting renderer (empty grid, hangs the
modal), and the exhaustiveness sentinel at the bottom of
`PendingChoiceModal.renderContent()` will fail typecheck. Both must
land atomically.

**The atomic UI + JSON swap (one commit, ui-specialist scope):**

1. **`packages/ui/src/components/PendingChoiceModal.tsx`** — add a
   branch in `renderContent()` for `pendingChoice.type ===
   "choose_players_subset"`:
   - Multi-checkbox over `pendingChoice.selectablePlayerIds` (each
     player as a labeled checkbox; show player display name from
     `state.players[id]`'s metadata, fall back to the ID).
   - Local state for the in-progress selection (`Set<PlayerID>` or
     `PlayerID[]`).
   - Submit button emits `RESOLVE_CHOICE` with `choice:
     PlayerID[]`. The empty subset is valid per CRD 6.1.4 (the
     prompt is `optional: true`), so the submit button stays enabled
     even with zero selections.
   - Use `pendingChoice.prompt` as the modal heading ("Choose any
     number of players.").
   - Update the exhaustiveness sentinel at the bottom of
     `renderContent()` so it still narrows correctly — no extra
     work needed once the new branch is added (the type union
     already includes the variant).

2. **`packages/engine/src/cards/card-set-8.json`** — Beyond the
   Horizon's `actionEffects` should be:
   ```json
   "actionEffects": [
     {
       "type": "each_player",
   ...
   ```
   This replaces the 4 forced effects (opp-discard + opp-draw +
   self-discard + self-draw).

3. **Verify decompile score**: `pnpm decompile-cards --top 12`
   should show Beyond the Horizon at ≥0.85 (currently 0.54). The
   renderer is already wired to emit "Choose any number of players.
   They discard their hands and draw 3 cards each." for this shape
   (commit d16d379, decompile-cards.ts each_player handler).

4. **Smoke test the UI branch**: load Beyond the Horizon in the
   sandbox, play the card, verify the modal surfaces with two
   player checkboxes, the submit accepts any subset (including
   empty), and the resolved game state matches the chosen subset.

5. **Tests**: 5 new engine tests in `set5-set8.test.ts` already
   cover the reducer-side behavior; no new engine tests needed.
