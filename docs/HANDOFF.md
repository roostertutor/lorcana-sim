...
Agent splits:
- **server agent**: ✅ SHIPPED 2026-05-26. `lobbies.last_heartbeat_at`
  column + partial index + `'abandoned'` status documentation (schema.sql:
  765-799); `PATCH /lobby/:id/heartbeat` endpoint (routes/lobby.ts) +
  `recordLobbyHeartbeat` service (lobbyService.ts); abandoned-lobby lazy
  detection (`markLobbyAbandonedIfStale`) hooked into `getLobby` /
  `getLobbyInfo` / `listLobbies` / `joinLobby` / `resolveLobbyCode` so a
  stale > 60s waiting/lobby row flips to `'abandoned'` on the next read;
  `POST /game/:id/claim-win` endpoint (routes/game.ts) + `claimWin`
  service (gameService.ts) using the existing chess-clock grace window
  (`checkGraceExhausted`) as the disconnect threshold. 28 new tests
  across `lobbyService.test.ts` (+13) and new `gameService.claimWin.test.ts`
  (+15). Server-only — no UI / engine touched.
- **GUI agent** (me): heartbeat loop in `MultiplayerLobby` while waiting
  (PATCH `/lobby/:id/heartbeat` every ~30s); stale-lobby error surfacing
  (status `'abandoned'` on `/info` → "this lobby was abandoned" surface);
  `mp-game` localStorage redirect to `/replay/:id` when game finished
  while tab was closed.
- **gameboard-specialist**: connection banner (reads the already-exposed
  ...
  claim-win button UX (POSTs `/game/:id/claim-win` once the grace
  countdown hits 0; server-enforced precondition, so the button can stay
  optimistically clickable).

Sub-decisions made on the server side:
- **Disconnect threshold for claim-win**: reused the chess-clock grace
  window (per-player per-game, default 3 min from `MATCH_CLOCK_CONFIG`)
  rather than introducing a separate 2-min constant. One threshold to
  reason about across the lazy-grace and claim-win paths; the grace
  bank is already correctly ratcheted across multiple disconnect /
  reconnect cycles. Spec said "2-min opponent-disconnect"; ratcheted
  3-min grace is the existing source of truth and behaves the same way.
- **`outcome_reason` enum**: reused the existing `'disconnect'` value
  (added in the 2026-05-18 chess-clock migration) for claim-win
  finalization. Semantically identical to a lazy-grace-driven loss;
  no parallel `'opponent_disconnect'` value needed.
- **Realtime broadcast on claim-win**: mirrors `finalizeClockLoss`'s
  shape — `games.status='finished'` + `state.wonBy='disconnect'` UPDATE
  triggers the existing postgres-changes Realtime channel
  (REPLICA IDENTITY FULL). No new event type. The disconnected
  opponent sees the game as completed via the same GET `/game/:id`
  path the rest of the game-over flow already uses.

Sequence: server done; gameboard-specialist + GUI agent follow-ups
