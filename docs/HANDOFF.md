...
- `docs/CRD_SNAPSHOT.txt` — current snapshot of v2.2.0 (Effective July 9,
  2026), 2617 lines. Replaced wholesale on each revision.
...
## Engine agent: work the CRD 2.2.0 review queue (START WHEN CURRENT TASK DONE)

**Trigger:** the generalist reconciled the tracker + snapshot to CRD v2.2.0 on
2026-07-11 (commits `7939d68`, `f12845a`). Snapshot regenerated, Set 13 Shift
variants reconciled from provisional TBD → official numbers. **Next: verify the
new/reworded rules against the engine.** Pick this up when the current engine
task is finished.

**Owner:** engine-expert.

**Where:** `docs/CRD_TRACKER.md` → "CRD 2.2.0 changes (reconciled from 2.0.1)"
section, second table. Rows marked **`review`** are text-shape reconciliations
ONLY — the CRD wording changed but no handler has been re-traced or re-tested.
Per CLAUDE.md "Handler existence is not correctness," each needs a code read +
regression test before flipping to ✅ (or 🐛 + fix if the engine diverges).
Read full rule text from `docs/Disney-Lorcana-Comprehensive-Rules-2.2.0-EN.pdf`;
cite `file:line` for every card/handler claim.

**Priority order (highest behavior-risk first):**

1. **1.9.5 — damage reduced to 0 = "not dealt damage."** Verify "whenever dealt
   damage" triggers do NOT fire when modifiers reduce damage to 0. Most likely
   real bug. Test: inject a damage-reduction modifier + a `dealt_damage` trigger,
   assert no fire.
2. **4.6.6.1 — negative Strength counts as 0 for challenge damage.** Confirm the
   challenge-damage calc clamps negative effective S to 0. Test a debuff pushing
   S below 0.
3. **1.9.4.1–.3 — damage-calc pipeline** (base → apply modifiers any order →
   total). Trace challenge + effect damage paths; confirm ordering + add a
   modifier-ordering test if none exists.
4. **4.5.1.2 / 4.6.4.3 — quest/challenge now "pay any costs" + check limiters.**
   No current card costs ink/exert to quest/challenge (forward-looking) — confirm
   the resolution step could support a cost without crashing; note gap vs
   no-card-yet.
5. **4.6.4.4 — readied challenger stays the challenging character.** Verify the
   challenging-character id is sticky if readied mid-challenge.
6. **4.7.3.2–.4 — move-a-character cost overhaul** (alternate move cost + payment
   modifiers). Check location-move mirrors play-cost.
7. **6.1.16.1–.3 — conditional dependent effects** generalized from static-only
   (old 6.4.4.x) to all effects. Audit `conditional_effect` coverage for
   non-static triggers/activateds.
8. **5.6.6 — characters remain in play when their location leaves play.** Verify
   location banish doesn't cascade-banish characters.

`⚠️`-marked rows (1.9.6, 6.4.3, 6.7.9, 5.6.5.1, 1.8.5) are renumber/nomenclature
— confirm-and-leave, no code change expected. The Set 13 Shift rows are already
✅ with existing tests — don't re-litigate; trust the reconciliation unless a
test actually fails.

**Per row:** flip status to ✅ (verified + tested) or 🐛 (fix landed), cite the
handler `file:line` + test name. Follow the bug-fix workflow (regression test +
audit improvement together). `pnpm test` green before claiming done. Delete this
entry when the queue is cleared.
